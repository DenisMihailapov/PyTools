...
class AudioFolder(Dataset):
    ...
    def get_dataloader(self, batch_size=1, shuffle=False, num_workers=0):
    ...
    def spectrogram(self, index, show = True, save = False, dpi=30, path = ''):
        ...
        audio = self.load_audio(index).numpy()
    ...
    def play_ind(self, index, sample_rate=None):
        audio = self.load_audio(index)
        if not sample_rate: sample_rate = self.sample_rate
        return Player(audio, rate=sample_rate)
